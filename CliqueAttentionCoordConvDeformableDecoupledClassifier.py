...
class CliqueFPN():
    ...
    def __init__(self,num_classes,num_anchors,batch_size,max_box_per_image,max_grid,ignore_thresh=0.6,learning_rate=0.01):
    ...
    def __build(self):
        ...
        self.num_block = {'1':4,'2':6,'3':6,'4':6}#4,6,6,6
        self.K = {'1':48,'2':48,'3':48,'4':48}
        self.use_decoupled = False
        self.use_deformconv = False
        ...
        if Detection_or_Classifier=='classifier':
            with tf.variable_scope('zsc_classifier'):
                x2 = bn(skip_2, self.is_training, name='batch_norm_x2')
                x3 = bn(skip_3, self.is_training, name='batch_norm_x3')
                x4 = bn(skip_4, self.is_training, name='batch_norm_x4')
                x2 = tf.reduce_mean(x2,[1,2],keep_dims=True)
                x3 = tf.reduce_mean(x3,[1,2],keep_dims=True)
                x4 = tf.reduce_mean(x4,[1,2],keep_dims=True)
                global_pool = tf.concat([x2,x3,x4],axis=-1)
                self.classifier_logits = tf.reshape(_conv_block('Logits',global_pool,self.num_classes,1,1,'SAME',False,None,None,self.is_training),
...
def CliqueBlock(name,x,num_block=6,num_filters=80,use_decoupled=True,norm='group_norm',activate='selu',is_training=True,use_deformconv=True,loop=1):
    with tf.variable_scope(name):
        ...
        for i in range(num_block):
            _x = _B_conv_block('_B_conv_{}_0'.format(i),x,4*num_filters,1,1,'SAME',use_decoupled,norm,activate,is_training)
        ...
        with tf.variable_scope('loop'):
            for i in range(num_block):
                del FeatureList[0]
                _x = _B_conv_block('_II_B_conv_{}_0'.format(i),tf.concat(FeatureList,axis=-1),4*num_filters,1,1,'SAME',use_decoupled,norm,activate,is_training)
        ...
        C = x1.shape.as_list()[-1]
        SE0 = tf.reduce_mean(x0,[1,2],keep_dims=True)
        SE1 = tf.reduce_mean(x1,[1,2],keep_dims=True)
        SE_input = tf.reduce_mean(input,[1,2],keep_dims=True)
        SE = tf.concat([SE_input,SE0,SE1],axis=-1)
        SE = _B_conv_block('SE_1',SE,C//16,1,1,'SAME',False,None,activate,is_training)
        SE = _B_conv_block('SE_2',SE,C,1,1,'SAME',False,None,activate,is_training)
        SE = tf.nn.sigmoid(SE)
...
def _deform_conv(name,x,num_filters=16,kernel_size=3,stride=1,offect=3,padding='SAME',use_decoupled=True,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        ...
        b = tf.get_variable('deform_conv_bias',[num_filters],tf.float32,initializer=tf.constant_initializer(0.000))
...
def SelfAttention(name,x,use_decoupled=True,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        ...
        C = x.get_shape().as_list()[-1]
        f = _B_conv_block('f',x,C//8,3,2,'SAME',use_decoupled,norm,activate,is_training)
        g = _B_conv_block('g',x,C//8,3,2,'SAME',use_decoupled,norm,activate,is_training)
        h = _B_conv_block('h',x,C,3,2,'SAME',use_decoupled,norm,activate,is_training)
        ...
        return x
##CompetitiveSE
def CompetitiveSE(name,x,attention,use_decoupled=True,norm='group_norm',activate='selu',is_training=True):
    with tf.variable_scope(name):
        C = x.get_shape().as_list()[-1]
        #SEnet channel attention
        weight_x = tf.reduce_mean(x,[1,2],keep_dims=True)#none,1,1,C
        weight_a = tf.reduce_mean(attention,[1,2],keep_dims=True)#none,1,1,C
        weight_c = tf.concat([weight_x,weight_a],axis=-1)
        weight_c = _B_conv_block('conv_1',weight_c,C//16,1,1,'SAME',use_decoupled,norm,activate,is_training)
        weight_c = _B_conv_block('conv_2',weight_c,C,1,1,'SAME',use_decoupled,norm,activate,is_training)
