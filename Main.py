...
def Main():
    ...
    if Mode=='Val':
        ...
        for epoch in range(StartEpoch,NumEpoch):
            ...
            top1 = Test(sess, model, test_loader)
            ...
            if batch % PrintFreq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'top_5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                   epoch, batch, dataloder.len()//BatchSize, loss=loss, top1=top1, top5=top5))
            batch += 1
            ...
            if batch % PrintFreq == 0:
                print('Epoch: [{0}][{1}/{2}]\t'
                  'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(
                   epoch, batch, dataloder.len()//BatchSize, loss=loss))
            batch += 1
    return sess, model

def Test(sess, model, dataloder):
